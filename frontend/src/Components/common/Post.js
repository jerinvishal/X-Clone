...
const Post = ({ post }) => {
	...
	const {mutate:commetPost,isPending:isCommenting}=useMutation({
		mutationFn:async()=>{
			try{
		...
		onSuccess:()=>{
			toast.success("Comemnt posted Sucessfully ");
			setComment("");
			//QueryClient.invalidateQueries({queryKey:["posts"]})
			...
			QueryClient.setQueryData(["posts"], (oldData) => {
				return oldData.map((p) => {
				  if (p._id === post._id) {
					return {
					  ...p,
					  comments: [...p.comments, { text: comment, user: authUser }], // Add new comment object
					};
				  }
				  return p;
				});
